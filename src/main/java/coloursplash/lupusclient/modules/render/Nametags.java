...
public class Nametags extends Module {
    ...
    private final Setting<Boolean> displayClient = sgPlayers.add(new BoolSetting.Builder()
            .name("client")
            .description("Shows which client the player is on.")
            .defaultValue(true)
            .build()
    );
    ...
    private final Setting<SettingColor> lupusColor = sgPlayers.add(new ColorSetting.Builder()
            .name("lupus-color")
            .description("The color of M when the player is using Lupus.")
    ...
    private void renderNametagPlayer(PlayerEntity player) {
        ...
        // Using Client
        String usingClient = "";
        if (displayClient.get() && MeteorPlayers.get(player)) usingClient = "M ";
        ...
        // Calc widths
        double usingClientWidth = text.getWidth(usingClient);
        ...
        if (displayClient.get()) width += usingClientWidth;
        ...
        if (displayClient.get()) hX = text.render(usingClient, hX, hY, lupusColor.get());
