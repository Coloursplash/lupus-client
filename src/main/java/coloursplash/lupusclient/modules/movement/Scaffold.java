...
public class Scaffold extends Module {
    ...
    private final Setting<Boolean> solidBlocks = sgBlocks.add(new BoolSetting.Builder()
            .name("solid-blocks-only")
            .description("Only use solid blocks.")
            .defaultValue(true)
            .build()
    );
    ...
    private int findSlot(BlockState blockState) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            // Filter out non solid blocks if solidBlocks is true
            Block block = ((BlockItem) stack.getItem()).getBlock();
            slotBlockState = block.getDefaultState();
            if (!Block.isShapeFullCube(slotBlockState.getCollisionShape(mc.world, setPos(0, -1, 0))) && solidBlocks.get()) continue;
