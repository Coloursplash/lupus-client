...
public class Waypoints extends System<Waypoints> implements Iterable<Waypoint> {
    ...
    @Override
    public void init() {
        File iconsFolder = new File(new File(LupusClient.FOLDER, "waypoints"), "icons");
        iconsFolder.mkdirs();
    ...
    public File getFile() {
        ...
        return new File(new File(LupusClient.FOLDER, "waypoints"), Utils.getWorldName() + ".nbt");
    }
    ...
    private void copyIcon(File file) {
        try {
            InputStream in = Waypoints.class.getResourceAsStream("/assets/lupus-client/waypoint-icons/" + file.getName());
